package com.app.todoapp.util

object Constants {
    const val Create : String = "Create"
    const val CreateNote : String = "Create note"
    const val Update : String = "Update"
    const val UpdateNote : String = "Update note"


    const val Channel_Name : String = "Todo Channel"
    const val Description : String = "A reminder."

    const val EmptyField : String = "Fields can't be empty"
    const val NoteAdded : String = "Note has been added!"
    const val NoteUpdate : String = "Note has been updated!"
    const val CreateNotification: String = "Notification has been created"
}